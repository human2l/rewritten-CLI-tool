...
const parameterAreaSteppingMode = document.getElementById(
  "parameter-area-stepping-mode"
);
const parameterAreaRealtimeMode = document.getElementById(
  "parameter-area-realtime-mode"
);
...
let steppingProcessingInterval = undefined;
...
function initControlDisplayState() {
  ...
  parameterAreaSteppingMode.hidden = true;
  parameterAreaRealtimeMode.hidden = false;
}
...
function initDefaultConfiguration() {
  ...
  videoWidth = "1080";
  mode = "realtimeMode";
  minimumSendingDelayInput.value = "1500";
  skipFramesInput.value = 100;
  ...
  inputFileElement.addEventListener(
    ...
    e => {
      ...
      video.oncanplay = function() {
        video.muted = true;
        //testing
        // var imageScaleRatio = 1 / 4;
        // video.height = video.videoHeight * imageScaleRatio;
        // video.width = video.videoWidth * imageScaleRatio;
        video.height = videoHeight; //or: video.videoHeight * imageScaleRatio
        video.width = videoWidth; //or: video.videoWidth * imageScaleRatio
...
function startAndSend() {
  ...
  if (mode == "realtimeMode") {
    ...
    skipFramesInput.disabled = true;
    waitTimeInput.disabled = true;
    startSteppingProcessingInterval();
...
function reset() {
  ...
  if (mode == "realtimeMode") {
    ...
    if (steppingProcessingInterval != undefined)
      clearInterval(steppingProcessingInterval);
...
function startRealtimeProcessingInterval() {
  ...
  }, minimumSendingDelayInput.value);
}
...
function startSteppingProcessingInterval() {
  //testing below
  processSteppingVideo();
}

function processSteppingVideo() {
  if (inputFileElement.files.item(0) == null) return;
  //   var currentTime;
  ...
  //   while (true) {
  //       console.log(previousDate+"now:"+currentDate)
  //     if (currentDate - previousDate >= waitTimeInput.value) {
  //       showFrames();
  //     } else {
  ...
  //   }
  // showFrames();
  //     setTimeout(run, waitTimeInput.value);
  //   }, waitTimeInput.value);

  var timeUsed = 0;
  //   console.log(waitTimeInput.value-timeUsed)
  setInterval(() => {
    showFrames();
    console.log(timeUsed);
  }, Number(waitTimeInput.value) + 2000);
  ...
  function showFrames() {
    ...
    timeUsed = endDate.getTime() - startDate.getTime();

    var videoCurrentTime = 0;
    // function steppingProcessing() {
    //   cap.read(frame);
    //   cv.imshow("uploadCanvas", frame);
    //   skipCounter++;
    //   if (skipCounter < skipFramesInput.value) {
    //     rAF = requestAnimationFrame(steppingProcessing);
    //   } else {
    //     cancelAnimationFrame(rAF);
    //     if(video.paused)return;
    //     videoCurrentTime = video.currentTime-videoCurrentTime
    ...
    setTimeout(function steppingProcessing() {
      cap.read(frame);
      cv.imshow("uploadCanvas", frame);
      skipCounter++;
      if (skipCounter < skipFramesInput.value) {
        setTimeout(steppingProcessing, 0);
      } else {
        video.pause();
        console.log("pause");
      }
...
function toggleMode(e) {
  ...
  if (mode == "realtimeMode") {
    ...
    parameterAreaSteppingMode.hidden = false;
    parameterAreaRealtimeMode.hidden = true;
  }
}
